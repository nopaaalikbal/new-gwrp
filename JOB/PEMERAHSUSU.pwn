...
RefreshMapJobSapi(playerid)
{
	...
	if(pData[playerid][pJob] == 5)
	{
		...
		{
			PemerasArea[playerid][Dutyarea] = CreateDynamicCircle(-65.75, 97.02, 1.0, -1, -1, playerid);
		 	PemerasArea[playerid][Dutycp] = CreateDynamicCP(-65.75, 97.02, 3.11, 2.0, -1, -1, playerid, 30.0);
		 	destroyladangsapi();
		}
		else
		{
			...
			PemerasArea[playerid][Olahsusu] = CreateDynamicCircle(-67.24, 81.01, 1.0, -1, -1, playerid);
			PemerasArea[playerid][Olahcp] = CreateDynamicCP(-67.24, 81.01, 3.13, 2.0, -1, -1, playerid, 30.0);

			PemerasArea[playerid][Iconsapi] = CreateDynamicMapIcon(-65.75, 97.02, 3.11, 19238, 0, -1, -1, playerid, 99999.0, MAPICON_GLOBAL);
			createladangsapi();
		}
		return 1;
	}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    {
        if (IsPlayerInRangeOfPoint(playerid, 2.0, -53.38, 98.24, 3.11))//sapi1
        {
        	if(pData[playerid][pSusu] == 30) return ErrorMsg(playerid, "Inventory susu Anda Sudah Penuh (MAX: 30 susu)");
            ...
            ShowProgressbar(playerid, "Take a Milk", 10);
            ApplyAnimation(playerid, "BOMBER","BOM_Plant_Loop",4.0, 1, 0, 0, 0, 0, 1); // Place Bomb
        }
        if (IsPlayerInRangeOfPoint(playerid, 2.0, -54.52, 95.16, 3.11))//sapi2
        {
        	if(pData[playerid][pSusu] == 30) return ErrorMsg(playerid, "Inventory susu Anda Sudah Penuh MAX: 30 susu");
        	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
            TogglePlayerControllable(playerid, 0);
        ...
        if (IsPlayerInRangeOfPoint(playerid, 2.0, -55.76, 91.75, 3.11))//sapi3
        {
        	if(pData[playerid][pSusu] == 30) return ErrorMsg(playerid, "Inventory susu Anda Sudah Penuh MAX: 30 susu");
        	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
            TogglePlayerControllable(playerid, 0);
        ...
        if (IsPlayerInRangeOfPoint(playerid, 2.0, -56.77, 88.60, 3.11))//sapi4
        {
        	if(pData[playerid][pSusu] == 30) return ErrorMsg(playerid, "Inventory susu Anda Sudah Penuh MAX: 30 susu");
        	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
            TogglePlayerControllable(playerid, 0);
        ...
        if (IsPlayerInRangeOfPoint(playerid, 2.0, -58.08, 85.69, 3.11))//sapi5
        {
        	if(pData[playerid][pSusu] == 30) return ErrorMsg(playerid, "Inventory susu Anda Sudah Penuh MAX: 30 susu");
        	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
            TogglePlayerControllable(playerid, 0);
        ...
        if(IsPlayerInRangeOfPoint(playerid, 4.0, -60.03, 80.20, 3.13))//olahsusu1
        {
        	return callcmd::olahsusu(playerid, "");
        }
        if(IsPlayerInRangeOfPoint(playerid, 4.0, -62.24, 78.77, 3.13))//olahsusu2
        {
        	return callcmd::olahsusu(playerid, "");
        }
        if(IsPlayerInRangeOfPoint(playerid, 4.0, -65.53, 79.95, 3.13))//olahsusu3
        {
        	return callcmd::olahsusu(playerid, "");
        }
        if(IsPlayerInRangeOfPoint(playerid, 4.0, -61.81, 75.34, 3.13))//olahsusu4
        {
        	return callcmd::olahsusu(playerid, "");
        }
    }
    else if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT && pData[playerid][pJob2] == 5)
	{
		if(IsPlayerInDynamicArea(playerid, PemerasArea[playerid][Dutyarea]))
		{
			if(!pData[playerid][pJobmilkduty])
			{
				...
				SuccesMsg(playerid, "Anda Sekarang Menggunakan Baju Pemerah");

				RefreshMapJobSapi(playerid);

				Info(playerid, "Silakan Dekatkan Sapi di Kandang Lalu Otot ALT");
			}
			else
			{
...
CMD:olahsusu(playerid, params[])
{
	...
	if(pData[playerid][pJob] == 5 || pData[playerid][pJob2] == 5)
	{
		new isValidLocation = false; // Menambahkan variabel untuk menandai apakah pemain berada di lokasi yang valid
		
		if(IsPlayerInRangeOfPoint(playerid, 4.0, -60.03, 80.20, 3.13)) //olahsusu1
		{
			isValidLocation = true;
		}
		else if(IsPlayerInRangeOfPoint(playerid, 4.0, -62.24, 78.77, 3.13)) //olahsusu2
		{
			isValidLocation = true;
		}
		else if(IsPlayerInRangeOfPoint(playerid, 4.0, -65.53, 79.95, 3.13)) //olahsusu3
		{
			isValidLocation = true;
		}
		else if(IsPlayerInRangeOfPoint(playerid, 4.0, -61.81, 75.34, 3.13)) //olahsusu4
		{
			isValidLocation = true;
		}

		if(isValidLocation) // Jika pemain berada di lokasi yang valid
		{
			if(pData[playerid][pLoading]) // Memeriksa apakah pemain sedang mengolah susu
				return ErrorMsg(playerid, "Anda masih sedang mengolah susu!");
			
			if(pData[playerid][pProgress] == 1) 
				return ErrorMsg(playerid, "Anda masih memiliki progres kegiatan!");
			
			if(pData[playerid][pSusu] < 1) 
				return ErrorMsg(playerid, "Anda harus mengambil susu terlebih dahulu!");
		...
		else // Jika pemain tidak berada di lokasi yang valid
		{
			return ErrorMsg(playerid, "Anda tidak berada di tempat pemeras susu.");
		}
	}
	else return ErrorMsg(playerid, "Anda bukan pekerja pemeras susu!");
...
CMD:jualsusuden(playerid, params[])//jualsusu
{
    if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
    ...
    ShowProgressbar(playerid, "Selling Milk", total);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new pay = pData[playerid][pSusuOlahan] * 40;
