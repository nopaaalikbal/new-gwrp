function MuatBarang(playerid)
{
	...
	InfoMsg(playerid, "Bawa muatan kembali ke ~y~Pelabuhan Greenwich ~w~ikutilah tanda.");
...
CMD:kargodentwot(playerid, params[])
{
    if(pData[playerid][pJob] == 8)
	{
	    if(delaykargo > 0) return ErrorMsg(playerid, "Mohon tunggu selama 200 detik");
		if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
		ShowPlayerDialog(playerid, DIALOG_KARGO, DIALOG_STYLE_TABLIST_HEADERS, "{BABABA}Greenwich - {FFFFFF}Pilih Kargo", "Jenis\nBarang\nPertamina", "Pilih", "Tutup");
	}
	return 1;
}
function KargoMinyak(playerid)
{
    if(IsPlayerConnected(playerid) && pData[playerid][pJob] == 8)
	{
	    ...
	    if(rand == 1)
	    {
	        pData[playerid][pKendaraanKerja] = CreateVehicle(515,-1030.085327,-731.758789,33.008674,358.277282,0,0,-1);
			...
			pData[playerid][pTrailer] = CreateVehicle(584,-1030.152465,-741.635375,32.007812,358.277282,0,0,-1);
...
function KargoBarang(playerid)
{
    if(IsPlayerConnected(playerid) && pData[playerid][pJob] == 8)
	{
	    ...
	    if(rand == 1)
	    {
	        pData[playerid][pKendaraanKerja] = CreateVehicle(515,-1029.995971,-732.025512,33.029350,358.277282,0,0,-1);
			PutPlayerInVehicle(playerid, pData[playerid][pKendaraanKerja], 0);
    		SetVehicleNumberPlate(pData[playerid][pKendaraanKerja], "JOB VEHICLE");
    		delaykargo = 200;
    		SetTimerEx("TungguKargo", 10000, false, "d", playerid);
			pData[playerid][pTrailer] = CreateVehicle(435,-1030.152465,-741.635375,32.007812,358.277282,0,0,-1);
