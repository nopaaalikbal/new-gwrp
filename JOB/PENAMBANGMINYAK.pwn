...
CMD:kerjaminyak1(playerid, params[])
{
	...
	if(pData[playerid][pMinyak] > 100) return ErrorMsg(playerid, "Anda tidak dapat membawa lebih dari 100 minyak!");
	if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress");
	ShowProgressbar(playerid, "Pickup Oil", 15);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	SetTimerEx("Minyak1", 15000, false, "d", playerid);
	return 1;
}
...
CMD:saringminyak(playerid, params[])
{
	...
	ShowProgressbar(playerid, "Filter Oil", 15);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	SetTimerEx("Minyak1", 10000, false, "d", playerid);
...
CMD:jualminyakden(playerid, params[])
{
	if(pData[playerid][pVip] > 0)
	{
	    ...
	    ShowProgressbar(playerid, "Selling Oil", 6);
		ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	...
	else
	{
	    ...
	    ShowProgressbar(playerid, "Selling Oil", 6);
		ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
		new pay = pData[playerid][pEssence] * 13;
		GivePlayerMoneyEx(playerid, pay);
		pData[playerid][pEssence] -= total;
		new str[500];
		format(str, sizeof(str), "Received_%dx", pay);
