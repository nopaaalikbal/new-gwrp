...
House_Type(houseid)
{
	if(hData[houseid][hType] == 1)
	{
	    switch(random(3))
		{
			case 0:
			{
				hData[houseid][hIntposX] = 2167.355957;
				hData[houseid][hIntposY] = 1104.735229;
				hData[houseid][hIntposZ] = 10110.449218;
	...
	if(hData[houseid][hType] == 2)
	{
	    switch(random(5))
		{
			case 0:
			{
				hData[houseid][hIntposX] = 1314.871948;
				hData[houseid][hIntposY] = 747.673217;
				hData[houseid][hIntposZ] = 1058.793212;
				hData[houseid][hIntposA] = 80.059730;
				hData[houseid][hInt] = 1;
			}
			case 1:
			{
	...
	if(hData[houseid][hType] == 3)
	{
	    switch(random(2))
		{
			case 0:
			{
				hData[houseid][hIntposX] = 1390.306884;
				hData[houseid][hIntposY] = 2812.753417;
				hData[houseid][hIntposZ] = 10011.320312;
				hData[houseid][hIntposA] = 0;
				hData[houseid][hInt] = 1;
			}
			case 1:
	...
	if(hData[houseid][hType] == 4)
	{
	    switch(random(2))
		{
			case 0:
			{
				hData[houseid][hIntposX] = 1024.449096;
				hData[houseid][hIntposY] = -1506.214721;
				hData[houseid][hIntposZ] = -12.170427;
				hData[houseid][hIntposA] = 2.026033;
				hData[houseid][hInt] = 1;
			}
			case 1:
			{
...
House_Refresh(houseid)
{
    if(houseid != -1)
    {
		...
		{
			format(string, sizeof(string), ""LG_E"[F]"WHITE_E"Rumah %s", hData[houseid][hOwner]);
			...
			hData[houseid][hPickup2] = CreateDynamicObject(1317, hData[houseid][hIntposX], hData[houseid][hIntposY], hData[houseid][hIntposZ]-1.0, 0.0, 0.0, 0.0, houseid, hData[houseid][hInt], -1, 50.00, 50.00);
			SetDynamicObjectMaterial(hData[houseid][hPickup2], 0, -1, "none", "none", 0xFFFFCCFF);
			hData[houseid][hMapIcon] = CreateDynamicMapIcon(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ], 32, 1, -1, -1, -1, 100.0);
		}
        else
        {
            format(string, sizeof(string), "[ID: %d]\n{00FF00}This house for sell\n{FFFFFF}House lokasi: {FFFF00}%s\n{FFFFFF}House type: {FFFF00}%s\n{FFFFFF}House harga: {FFFF00}%s\n"WHITE_E"Type {49ee4f}/buy {ffffff}Untuk membeli rumah", houseid, GetLocation(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ]), type, FormatMoney(hData[houseid][hPrice]));
			...
			hData[houseid][hPickup2] = CreateDynamicObject(1317, hData[houseid][hIntposX], hData[houseid][hIntposY], hData[houseid][hIntposZ]-1.0, 0.0, 0.0, 0.0, houseid, hData[houseid][hInt], -1, 50.00, 50.00);
			SetDynamicObjectMaterial(hData[houseid][hPickup2], 0, -1, "none", "none", 0xFFFFCCFF);
			hData[houseid][hMapIcon] = CreateDynamicMapIcon(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ], 31, 1, -1, -1, -1,100.0);
		}
...
GetHouseStorage(houseid, item)
{
	static const StorageLimit[][] = {
	   //Snack  Sprunk  Medicine  Medkit  Bandage  Seed  Material  Component  Marijuana
	    {35,    35,     30,  	  10,  	  15,  	   255,  255,  	   255, 	 150},// Small
...
function OnHousesCreated(playerid, hid)
{
	House_Type(hid);
    House_Refresh(hid);
	Servers(playerid, "House [%d] Success To Create!", hid);
	new str[150];
	format(str,sizeof(str),"[House]: %s Create House ID %d!", GetRPName(playerid), hid);
	LogServer("Admin", str);
	return 1;
}
...
CMD:sellhouse(playerid, params[])
{
	...
	if(GetOwnedHouses(playerid) == -1) return ErrorMsg(playerid, "You Don't Has House.");
	...
	ShowPlayerDialog(playerid, DIALOG_SELL_HOUSES, DIALOG_STYLE_LIST, "{27d23d}Greenwich {ffffff}/{27d23d}/ {ffffff}Sell House", CMDSString, "{27d23d}Sell", "{e31d1d}Cancel");
	return 1;
}
...
CMD:myhouse(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_HOUSES, DIALOG_STYLE_LIST, "{27d23d}Greenwich {ffffff}/{27d23d}/ {ffffff}House", CMDSString, "{27d23d}Enter", "{ffffff}Cancel");
	return 1;
}
