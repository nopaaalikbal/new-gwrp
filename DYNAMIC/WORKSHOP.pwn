...
#define MAX_WORKSHOP_EMPLOYEE 4
#define MAX_WORKSHOP_INT 10000
...
new wsData[MAX_WORKSHOP][E_WORKSHOP],
    wsEmploy[MAX_WORKSHOP][4][MAX_PLAYER_NAME + 1],
...
IsWorkshopEmploye(playerid, id)
{
    ...
    if(!strcmp(wsEmploy[id][3], pData[playerid][pName], true)) return 1;
    return 0;
}
    ...
    foreach(new id : Workshop)
	{
        if(IsPlayerInRangeOfPoint(playerid, 4.0, wsData[id][wX], wsData[id][wY], wsData[id][wZ]))
        {
            if(!IsWorkshopOwner(playerid, id) && !IsWorkshopEmploye(playerid, id))
                return ErrorMsg(playerid, "Anda bukan pemilik atau pegawai workshop");
