...
Bisnis_BuyMenu(playerid, bizid)
{
    ...
    switch(bData[bizid][bType])
    {
        case 1:
        {
            ...
            ShowPlayerDialog(playerid, BISNIS_BUYPROD, DIALOG_STYLE_TABLIST_HEADERS, bData[bizid][bName], string, "{27d23d}Buy", "{e31d1d}Cancel");
        }
       case 2:
        {
            format(string, sizeof(string), "Product\tPrice\n{ffffff}Snack\t{7fff00}%s\n{ffffff}Water\t{7fff00}%s\n{ffffff}Jerigen\t{7fff00}%s\n{ffffff}Bandage\t{7fff00}%s",
                FormatMoney(bData[bizid][bP][0]),
                FormatMoney(bData[bizid][bP][1]),
                FormatMoney(bData[bizid][bP][2]),
                FormatMoney(bData[bizid][bP][3]),
            ...
            ShowPlayerDialog(playerid, BISNIS_BUYPROD ,DIALOG_STYLE_TABLIST_HEADERS, "{27d23d}Business {ffffff}/{27d23d}/ {ffffff}Price Products", string, "{27d23d}Enter", "{e31d1d}Cancel");
        }
        case 3:
        {
            TextDrawShowForPlayer(playerid, BajuTD[0]);
			TextDrawShowForPlayer(playerid, BajuTD[1]);
			TextDrawShowForPlayer(playerid, BajuTD[2]);
			TextDrawShowForPlayer(playerid, BajuTD[3]);
			TextDrawShowForPlayer(playerid, BajuTD[8]);
			TextDrawShowForPlayer(playerid, BajuTD[9]);
			TextDrawShowForPlayer(playerid, BajuTD[10]);
			TextDrawShowForPlayer(playerid, BajuTD[11]);
			TextDrawShowForPlayer(playerid, BajuTD[12]); 
...
Bisnis_ProductMenu(playerid, bizid)
{
    ...
    switch (bData[bizid][bType])
    {
        ...
        case 2:
        {
            format(string, sizeof(string), "Product\tAmount\n{ffffff}Snack\t{7fff00}%s\n{ffffff}Water\t{7fff00}%s\n{ffffff}Jerigen\t{7fff00}%s\n{ffffff}Bandage\t{7fff00}%s",
                FormatMoney(bData[bizid][bP][0]),
                FormatMoney(bData[bizid][bP][1]),
                FormatMoney(bData[bizid][bP][2]),
                FormatMoney(bData[bizid][bP][3]),
            ...
            ShowPlayerDialog(playerid, BISNIS_EDITPROD,DIALOG_STYLE_TABLIST_HEADERS, "{27d23d}Business {ffffff}/{27d23d}/ {ffffff}Harga Barang", string, "{27d23d}Change", "{ffffff}Cancel");
        }
        case 3:
        {
...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
		...
		{
			format(string, sizeof(string), "Owner: {FFFF00}%s\n"WHITE_E"Business Name: %s\nPress "LG_E"'ALT' {FFFFFF}For Buy Business", bData[id][bOwner], bData[id][bName]);
        }
        else
        {
            format(string, sizeof(string), "Business Is Sale\nPrice: {FFFF00}%s\n{FFFFFF}Type: {FFFF00}%s\n"WHITE_E"Press "LG_E"N {FFFFFF}For Buy Business\n"LG_E"ALT {FFFFFF}For Buy", FormatMoney(bData[id][bPrice]), type);
		}
		bData[id][bPickPoint] = CreateDynamicObject(1317, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ] -1.0, 0.000000, 0.000000, 0.000000, -1, -1, -1, 300.00, 300.00);
		SetDynamicObjectMaterial(bData[id][bPickPoint], 0, -1, "none", "none", 0xFFCCCC00);
		//bData[id][bCP] = CreateDynamicCP(bData[id][bIntposX], bData[id][bIntposY], bData[id][bIntposZ], 1.0, id, bData[id][bInt], -1, 3.0);
        bData[id][bLabel] = CreateDynamic3DTextLabel(string, COLOR_WHITE, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0);
		...
		{
			bData[id][bMap] = CreateDynamicMapIcon(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 35, -1, -1, -1, -1, 700.0);
		}
		else
		{
			DestroyDynamicMapIcon(bData[id][bMap]);
...
CMD:createbiz(playerid, params[])
{
	...
	new price, type;
	if(sscanf(params, "dd", price, type)) return SyntaxMsg(playerid, "/createbiz [price] [type, 1.Fastfood 2.Market 3.Clothes 4.Equipment 5.Electronics]");
...
function OnBisnisCreated(playerid, bid)
{
	Bisnis_Type(bid);
    Bisnis_Refresh(bid);
	Servers(playerid, "Business [%d] Success To Created!", bid);
	new str[150];
	format(str,sizeof(str),"[Biz]: %s Create Business ID %d!", GetRPName(playerid), bid);
	LogServer("Admin", str);
	return 1;
}
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1376.164062,1573.931030,17.000314)) return ErrorMsg(playerid, "You Must In Greenwich City Hall!");
	...
	ShowPlayerDialog(playerid, DIALOG_SELL_BISNISS, DIALOG_STYLE_LIST, "{007a1e}Sell Business", CMDSString, "{27d23d}Sell", "{e31d1d}Cancel");
	return 1;
}

CMD:mybiz(playerid)
{
	if(GetOwnedBisnis(playerid) == -1) return ErrorMsg(playerid, "You Don't Has Business.");
	//if(!Player_OwnsBusiness(playerid, id)) return Error(playerid, "You don't own this business.");
	new hid, _tmpstring[128], count = GetOwnedBisnis(playerid), CMDSString[512];
	CMDSString = "";
	new lock[128];
	Loop(itt, (count + 1), 1)
	{
		...
		if(hData[hid][hLocked] == 1)
		{
			lock = "{FF0000}Locked{ffffff}";
		
		}
		else
		{
			lock = "{00FF00}Unlocked{ffffff}";
		}
		if(itt == count)
		{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "{27d23d}Greenwich {ffffff}/{27d23d}/ {ffffff}My Business", CMDSString, "{27d23d}Enter", "{e31d1d}Cancel");
	return 1;
}
...
CMD:bm(playerid, params[])
{
	...
	if(!Player_OwnsBisnis(playerid, pData[playerid][pInBiz])) return ErrorMsg(playerid, "Kamu tidak memiliki bisnis ini.");
    ShowPlayerDialog(playerid, BISNIS_MENU, DIALOG_STYLE_LIST, "{27d23d}Greenwich {ffffff}/{27d23d}/ {ffffff}Bisnis Menu","Business Info\nChange Name\nBusiness Income\nPrice Product\nRequest Restock","{27d23d}Enter","{e31d1d}Close");
    return 1;
}
...
{
	new bid, otherid;
	if(sscanf(params, "ud", otherid, bid)) return SyntaxMsg(playerid, "/givebiz [playerid/name] [id] | /mybiz - for show info");
