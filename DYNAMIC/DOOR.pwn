...
Doors_Updatelabel(id)
{
	if(id != -1)
	{
		...
		{
			new mstr[512];
			format(mstr,sizeof(mstr),"[ID: %d]\n{BABABA}%s", id, dData[id][dName]);
			dData[id][dPickupext] = CreateDynamicObject(1317, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ]-1.0, 0.0, 0.0, 0.0, dData[id][dExtvw], dData[id][dExtint], -1, 50.00, 50.00);
			SetDynamicObjectMaterial(dData[id][dPickupext], 0, -1, "none", "none", 0xFF00FFFF);
		...
		else
		{
			...
			format(mstr,sizeof(mstr),"[ID: %d]\n{BABABA}%s", id, dData[id][dName]);
			dData[id][dPickupext] = CreateDynamicObject(1317, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ]-1.0, 0.0, 0.0, 0.0, dData[id][dExtvw], dData[id][dExtint], -1, 50.00, 50.00);
			SetDynamicObjectMaterial(dData[id][dPickupext], 0, -1, "none", "none", 0xFFFFCCFF);
			dData[id][dLabelext] = CreateDynamic3DTextLabel(mstr, COLOR_YELLOW, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ]+0.35, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, dData[id][dExtvw], dData[id][dExtint]);
		}
        ...
        if(dData[id][dIntposX] != 0.0 && dData[id][dIntposY] != 0.0 && dData[id][dIntposZ] != 0.0)
        {
			if(dData[id][dGarage] == 1)
			{
				...
				dData[id][dPickupint] = CreateDynamicObject(1317, dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ]-1.0, 0.0, 0.0, 0.0, dData[id][dIntvw], dData[id][dIntint], -1, 50.00, 50.00);
				SetDynamicObjectMaterial(dData[id][dPickupint], 0, -1, "none", "none", 0xFF00FFFF);
			}
			else
			{
				new mstr[512];
				...
				dData[id][dLabelint] = CreateDynamic3DTextLabel(mstr, COLOR_YELLOW, dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ]+0.7, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, dData[id][dIntvw], dData[id][dIntint]);
				dData[id][dPickupint] = CreateDynamicObject(1317, dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ]-1.0, 0.0, 0.0, 0.0, dData[id][dIntvw], dData[id][dIntint], -1, 50.00, 50.00);
				SetDynamicObjectMaterial(dData[id][dPickupint], 0, -1, "none", "none", 0xFFFFCCFF);
			}
		}
...
function OnDoorsCreated(playerid, id)
{
	Doors_Save(id);
	Doors_Updatelabel(id);
	Servers(playerid, "{FFB6C1}Door [%d] berhasil di buat!", id);
	return 1;
}
...
CMD:createdoor(playerid, params[])
{
	...
	format(mstr,sizeof(mstr),"[ID: %d]\n{BABABA}%s", did, dData[did][dName]);
	dData[did][dPickupext] = CreateDynamicPickup(19606, 23, dData[did][dExtposX], dData[did][dExtposY], dData[did][dExtposZ], dData[did][dExtvw], dData[did][dExtint], -1, 50);
...
CMD:gotodoor(playerid, params[])
{
	...
	new str[500];
	format(str,sizeof(str),"Anda teleport ke pintu ~p~%s", dData[did][dName]);
...
CMD:editdoor(playerid, params[])
{
    ...
    {
        SyntaxMsg(playerid, "/editdoor [id] [name]");
        SendClientMessage(playerid, COLOR_YELLOW, "{FFB6C1}[NAMES]:{BABABA} location, interior, password, name, locked, admin, vip, faction, family, custom, virtual, iconmap");
        return 1;
    }
    if((did < 0 || did >= MAX_DOOR))
        return ErrorMsg(playerid, "You have specified an invalid entrance ID.");
	if(!Iter_Contains(Doors, did)) return Error(playerid, "The doors you specified ID of doesn't exist.");
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendAdminMessage(COLOR_RED, "{FFB6C1}%s has adjusted the location of entrance ID: %d.", pData[playerid][pAdminname], did);
    }
    else if(!strcmp(type, "interior", true))
    {
       ...
       /*foreach (new i : Player)
        ...
        SendAdminMessage(COLOR_RED, "{FFB6C1}%s has adjusted the interior spawn of entrance ID: %d.", pData[playerid][pAdminname], did);
    }
    else if(!strcmp(type, "custom", true))
    {
        ...
        if(status) {
            SendAdminMessage(COLOR_RED, "{FFB6C1}%s has enabled custom interior mode for entrance ID: %d.", pData[playerid][pAdminname], did);
        }
        else {
            SendAdminMessage(COLOR_RED, "{FFB6C1}%s has disabled custom interior mode for entrance ID: %d.", pData[playerid][pAdminname], did);
        }
    }
    else if(!strcmp(type, "virtual", true))
    {
        ...
        if(sscanf(string, "d", worldid))
            return SyntaxMsg(playerid, "/editdoor [id] [virtual] [interior world]");

        dData[did][dExtvw] = worldid;

        Doors_Save(did);
		Doors_Updatelabel(did);
        SendAdminMessage(COLOR_RED, "{FFB6C1}%s has adjusted the virtual of entrance ID: %d to %d.", pData[playerid][pAdminname], did, worldid);
    }
    else if(!strcmp(type, "password", true))
    {
        ...
        if(sscanf(string, "s[32]", password))
            return SyntaxMsg(playerid, "/editdoor [id] [password] [entrance pass] (use 'none' to disable)");
        ...
        Doors_Save(did);
		Doors_Updatelabel(did);
        SendAdminMessage(COLOR_RED, "{FFB6C1}%s has adjusted the password of entrance ID: %d to %s", pData[playerid][pAdminname], did, password);
    }
    else if(!strcmp(type, "locked", true))
    {
        ...
        if(locked) {
            SendAdminMessage(COLOR_RED, "{FFB6C1}%s has locked entrance ID: %d.", pData[playerid][pAdminname], did);
        } else {
            SendAdminMessage(COLOR_RED, "{FFB6C1}%s has unlocked entrance ID: %d.", pData[playerid][pAdminname], did);
        }
    }
    else if(!strcmp(type, "name", true))
    {
        ...
        SendAdminMessage(COLOR_RED, "{FFB6C1}%s has adjusted the name of entrance ID: %d to \"%s\".", pData[playerid][pAdminname], did, ColouredText(name));
    }
	else if(!strcmp(type, "admin", true))
    {
        ...
        SendAdminMessage(COLOR_RED, "{FFB6C1}%s has set entrance ID: %d to admin level %d.", pData[playerid][pAdminname], did, level);
    }
	else if(!strcmp(type, "vip", true))
    {
        ...
        SendAdminMessage(COLOR_RED, "{FFB6C1}%s has set entrance ID: %d to VIP level %d.", pData[playerid][pAdminname], did, level);
    }
	else if(!strcmp(type, "faction", true))
    {
        ...
        SendAdminMessage(COLOR_RED, "{FFB6C1}%s has set entrance ID: %d to faction id %d.", pData[playerid][pAdminname], did, fid);
    }
	else if(!strcmp(type, "family", true))
    {
        ...
        if(fid < -1 || fid > 20)
            return Error(playerid, "Invalid value. Use -1 - 20 for family id.");

        dData[did][dFamily] = fid;
        Doors_Save(did);
		Doors_Updatelabel(did);

        SendAdminMessage(COLOR_RED, "{FFB6C1}%s has set entrance ID: %d to family id %d.", pData[playerid][pAdminname], did, fid);
    }
	else if(!strcmp(type, "garage", true))
	{
		...
		{
			dData[did][dGarage] = 0;
			SendAdminMessage(COLOR_RED, "{FFB6C1}%s has set entrance ID: %d to garage vehicle disable.", pData[playerid][pAdminname], did);
		}
		else
		{
			dData[did][dGarage] = 1;
			SendAdminMessage(COLOR_RED, "{FFB6C1}%s has set entrance ID: %d to garage vehicle enable.", pData[playerid][pAdminname], did);
		}
		Doors_Save(gid);
	...
	else if(!strcmp(type, "iconmap", true))
	{
	    ...
	    {
	        return SyntaxMsg(playerid, "/editdoor [id] [iconmap] [iconid (0-63)]");
		}
		if(!(0 <= iconid <= 63))
		{
		    return ErrorMsg(playerid, "Ikon peta tidak valid..");
		}
	    ...
	    SendAdminMessage(COLOR_RED, "{FFB6C1}%s has set entrance ID: %d to Map Icon id %d.", pData[playerid][pAdminname], did, iconid);
	}
	else if(!strcmp(type, "delete", true))
    {
        ...
        SendAdminMessage(COLOR_RED, "{FFB6C1}%s has delete door ID: %d.", pData[playerid][pAdminname], did);
		new str[150];
