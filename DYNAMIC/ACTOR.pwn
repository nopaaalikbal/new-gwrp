...
function LoadActor()
{
	...
	if(rows)
	{
	    for(new i; i < rows; i++)
	    {
	        ...
	        new label[200];
	        format(label, sizeof label, "[Actor ID: %d]\n{BABABA}%s", aid, ActorData[aid][actorName]);
	        ...
	        ActorData[aid][actorText] = CreateDynamic3DTextLabel(label, COLOR_ARWIN, ActorData[aid][actorX], ActorData[aid][actorY], ActorData[aid][actorZ]+1,10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, ActorData[aid][actorWorld], ActorData[aid][actorInt], -1);
			ActorData[aid][actorWorld] = SetActorVirtualWorld(ActorData[aid][actorID], ActorData[aid][actorWorld]);
			ApplyActorAnimation(ActorData[aid][actorID], "COP_AMBIENT", "COPLOOK_LOOP", 4.1, 1, 0, 0, 0, 0);
            Iter_Add(Actors, aid);
		}
...
{
	Actor_Save(tid);
	Servers(playerid, "Actor [%d] Success To Create!", tid);
	new str[150];
	format(str,sizeof(str),"[Actor]: %s Create Actor ID %d!", GetRPName(playerid), tid);
	LogServer("Admin", str);
}
...
CMD:createactor(playerid, params[])
{
	...
	if (sscanf(params, "is[80]", skin, name))
		return SyntaxMsg(playerid, "/createactor [skin] [name]");

	if (skin < 0 || skin > 299)
	    return ErrorMsg(playerid, "Invalid skin ID. Skins range from 0 to 299.");

	new tid = Iter_Free(Actors), query[512];
	if(tid == -1) return ErrorMsg(playerid, "Actor Has Reached The Max Number");
    ...
    ActorData[tid][actorWorld] = GetPlayerInterior(playerid);

    SetPlayerPos(playerid, x + 5, y, z);

	new label[100];
	format(label, sizeof label, "%s", ActorData[tid][actorName]);
	...
	Iter_Add(Actors, tid);
	ApplyActorAnimation(ActorData[tid][actorID], "COP_AMBIENT", "COPLOOK_LOOP", 4.1, 1, 0, 0, 0, 0);
...
CMD:editactor(playerid, params[])
{
	...
	{
		SyntaxMsg(playerid, "/editactor [id] [name]");
		Info(playerid, "Names: Anim, Name, Skin, Pos, ClearAnim");
		return 1;
	}	
	if(!Iter_Contains(Actors, id)) return ErrorMsg(playerid, "Invalid ID.");

    if(!strcmp(name, "skin", true)) 
	{			
		if(sscanf(param, "i", ammount))
			return SyntaxMsg(playerid, "/editactor [id] [skin] [skin id]");
        ...
        new label[100];
	    format(label, sizeof label, "%s", ActorData[id][actorName]);
	...
	{			
		if(sscanf(param, "s[24]", text))
			return SyntaxMsg(playerid, "/editactor [id] [name] [name]");
...
CMD:deleteactor(playerid, params[])
{
	...
	new id, query[512];
	if(sscanf(params, "i", id)) return SyntaxMsg(playerid, "/deleteactor [id]");
	if(!Iter_Contains(Actors, id)) return ErrorMsg(playerid, "Invalid ID.");
...
CMD:gotoactor(playerid, params[])
{
	...
	if(sscanf(params, "d", id))
		return SyntaxMsg(playerid, "/gotoactor [id]");
	if(!Iter_Contains(Actors, id)) return ErrorMsg(playerid, "Actor ID tidak ada.");
