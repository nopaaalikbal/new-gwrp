...
CMD:createtrash(playerid, params[])
{
	...
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO trash SET id=%d, posx='%f', posy='%f', posz='%f', posa='%f', interior=%d, world=%d, sampah=%d", id, TrashData[id][TrashX], TrashData[id][TrashY], TrashData[id][TrashZ], TrashData[id][TrashA], GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid), TrashData[id][Sampah]);
