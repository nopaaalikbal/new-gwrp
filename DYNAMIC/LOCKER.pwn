...
CMD:createlocker(playerid, params[])
{
	...
	if(lid == -1) return ErrorMsg(playerid, "You cant create more locker!");
	new type;
	if(sscanf(params, "d", type)) return SendClientMessageEx(playerid, COLOR_WHITE, "[ ! ]: /createlocker [type, 1.GWPD 2.GWGS 3.GWMD 4.GWMD 5.FVENDORS 6.GWJEK 7.VIP 8.FAMILY]");
...
CMD:editlocker(playerid, params[])
{
    ...
    {
        SendClientMessageEx(playerid, COLOR_WHITE, "[USE]: /editlocker [id] [name]");
        SendClientMessage(playerid, COLOR_YELLOW, "[NAME]:{FFFFFF} location, type, delete");
        return 1;
    }
    ...
    else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", tipe))
            return SendClientMessageEx(playerid, COLOR_WHITE, "[USAGE]: /editlocker [id] [type] [type, 1.GWPD 2.GWGS 3.GWMD 4.GWNA 5.FVENDORS 6.VIP 7. FAMILY]");
