...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	if(cache_num_rows() > 0)
	{
		...
		{
			new str[400];
			format(str, sizeof(str), "UCP: {007a1e}%s\n{00fc19}Please enter the PIN that has been sent to you by Greenwich Bot.", pData[playerid][pUCP]);
			ShowPlayerDialog(playerid, DIALOG_VERIFYCODE, DIALOG_STYLE_INPUT, "{007a1e}Greenwich - {ffffff}Verify Account", str, "{27d23d}Enter", "{e31d1d}Cancel}");
		}
		if(pData[playerid][pPassword] > 10)
		{
			new lstring[512];
			format(lstring, sizeof lstring, "Welcome To {007a1e}Greenwich roleplay\n{ffffff}UCP Is Verified!\n\nUCP Name: {007a1e}%s\n{007a1e}({00fc19}Please enter your password below:{007a1e})", pData[playerid][pUCP]);
			ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "UCP - LOGIN", lstring, "{27d23d}Enter", "{e31d1d}Close");
		}
		//pData[playerid][LoginTimer] = SetTimerEx("OnLoginTimeout", SECONDS_TO_LOGIN * 1000, false, "i", playerid);
	}
	else
	{
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX,"{007a1e}Greenwich Roleplay - {BABABA}Tiket Karcis","{00fc19}Dari: Penjaga Pintu Greenwich #1\nKepada: Calon Aktor (Pemain Peran) di Kota Greenwich\n\nSilahkan Terlebih Dahulu mengambil tiket pintu Greenwich #1 di discord sebelum dapat memasuki Kota Greenwich\nLink Discord: bit.ly/Greenwich","{e31d1d}Close}","");
...
function UpdateNametagAdmin()
{
    for(new i = 0, j = GetPlayerPoolSize(); i <= j; i++)
    {
        if(IsPlayerConnected(i))
        {
            ...
            if(armour > 1.0)
            {
                format(nametag, sizeof(nametag), "{49EE4F}On Duty\n%s [%d]", pData[i][pAdminname], i);
            }
            else
            {
                format(nametag, sizeof(nametag), "{49EE4F}On Duty\n%s [%d]", pData[i][pAdminname], i);
            }
            UpdateDynamic3DTextLabelText(pData[i][AdminTag], 0xFFFFFFFF, nametag);
        }
    }
}
...
function YANGTIM(playerid)
{
	if(!STATUS_BOT2)
	{
		DCC_SetBotActivity("Greenwich ROLEPLAY MENGUDARA");
		STATUS_BOT2 = true;
	}
	else
	{
		DCC_SetBotActivity("Greenwich V3");
		STATUS_BOT2 = false;
	}
	return 1;
}
...
function AssignPlayerData(playerid)
{
	...
	//cache_get_value_name_int(0, "skck_time", pData[playerid][pIDCardTime]);
    ...
    cache_get_value_name_int(0, "sks", pData[playerid][pSks]);
	...
	{
		ShowPlayerDialog(playerid, DIALOG_SPAWNLENZ, DIALOG_STYLE_LIST, "{007a1e}Greenwich {ffffff}/{007a1e}/{ffffff} Spawn", "{007a1e}Fraksi {ffffff}> Spawn di depan fraksi\n{007a1e}Stasiun {ffffff}> Spawn di stasun kota\n{007a1e}Last Spawn {ffffff}> Spawn di lokasi terakhir", "Spawn", "");
	   	ClearChat(playerid);
	}
	...
	new streng[128];
	mysql_format(g_SQL, streng, sizeof(streng), "SELECT * FROM `bill` WHERE `bid` = '%d'", pData[playerid][pID]);
	mysql_tquery(g_SQL, streng, "LoadBill", "d", playerid);
	...
	DCC_SetEmbedTitle(logss, "Greenwich Roleplay | MoreThanCommunity");
	DCC_SetEmbedTimestamp(logss, "Penjaga pintu Greenwich");
	...
	DCC_SetEmbedFooter(logss, "Greenwich ROLEPLAY | MORETHANCOMMUNITY", "");
	new stroi[5000];
	format(stroi, sizeof(stroi), "**s** telah masuk ke dalam Pintu Greenwich #1\n**UCP: %s**\n**Player ID:** %d\n**Negara:** Indonesia", pData[playerid][pName], pData[playerid][pUCP], playerid);
	DCC_AddEmbedField(logss, "Penjaga pintu Greenwich", stroi, true);
...
function OnPlayerRegister(playerid)
{
	...
	pData[playerid][pPosX] = 1761.65;
	pData[playerid][pPosY] = -1897.75;
	pData[playerid][pPosZ] = 13.56;
	pData[playerid][pPosA] = 272.08;
	...
	new rend = RandomEx(111111, 999999), query[128];
	new rek = rend+pData[playerid][pID];
	mysql_format(g_SQL, query, sizeof(query), "SELECT brek FROM players WHERE brek='%d'", rek);
	mysql_tquery(g_SQL, query, "BankRek", "id", playerid, rek);
	
	SetSpawnInfo(playerid, NO_TEAM, 0, 1749.1129,-2516.4268,13.5969,85.07, 0, 0, 0, 0, 0, 0);
	SpawnPlayer(playerid);
	return 1;
}
...
{
	SetPlayerPositionEx(playerid, -2018.31,-186.55,39.73,96.81, 2000);
...
function CheckPlayerSpawn3Titik(playerid)
{
	...
	{
		SetPlayerPos(playerid, 1762.15,-1899.21,13.56);
