...
function CheckPlayerChar(playerid)
{
	...
	new query[256];
	format(query, sizeof(query), "SELECT `username`, `level`, `skin` FROM `players` WHERE `ucp`='%s' LIMIT %d;", pData[playerid][pUCP], MAX_CHARS + pData[playerid][pExtraChar]);
...
ShowCharacterList(playerid)
{
	...
	if(count < MAX_CHARS)
		strcat(name, ""LG_E"+ New Characer");
	strins(name, "Character Name\tCharacter Level\n", 0);
	ShowPlayerDialog(playerid, DIALOG_CHARLIST, DIALOG_STYLE_TABLIST_HEADERS, "{007a1e}Greenwich {ffffff}/{007a1e}/ {ffffff}List Character", name, "{27d23d}Enter", "{e31d1d}Close");
	return 1;
}
