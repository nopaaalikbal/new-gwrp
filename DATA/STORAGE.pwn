...
ShowStorageMenu(playerid, id)
{
    ...
    new str[256], vstr[64];
    format(vstr, sizeof vstr,"Greenwich - Storage (%s)", sData[id][sName]);
	...
	{
    ShowPlayerDialog(playerid, DIALOG_SG_MENU, DIALOG_STYLE_LIST, vstr, str, "{27d23d}Enter", "{e31d1d}Cancel");
 	}
    return 1;
}
...
CMD:editstorage(playerid, params[])
{
	...
	else if(!strcmp(type, "money", true))
    {
        ...
        Storage_Save(sid);
		Storage_Refresh(sid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the money of Storage ID: %d to %s.", pData[playerid][pAdminname], sid, FormatMoney(money));
    }
	else if(!strcmp(type, "comp", true))
    {
        ...
        Storage_Save(sid);
		Storage_Refresh(sid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the component of Storage ID: %d to %d.", pData[playerid][pAdminname], sid, amount);
    }
    else if(!strcmp(type, "mat", true))
    {
        ...
        Storage_Save(sid);
		Storage_Refresh(sid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the material of Storage ID: %d to %d.", pData[playerid][pAdminname], sid, amount);
    }
    else if(!strcmp(type, "owner", true))
    {
        ...
        Storage_Save(sid);
		Storage_Refresh(sid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the owner of Storage ID: %d to %s", pData[playerid][pAdminname], sid, owners);
    }
    else if(!strcmp(type, "reset", true))
    {
		...
		Storage_Save(sid);
		Storage_Refresh(sid);
        SendAdminMessage(COLOR_RED, "%s has reset Storage ID: %d.", pData[playerid][pAdminname], sid);
    }
...
CMD:mystorage(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_SG, DIALOG_STYLE_TABLIST_HEADERS, "My Storage", CMDSString, "{27d23d}Track", "{e31d1d}Cancel");
	return 1;
}
