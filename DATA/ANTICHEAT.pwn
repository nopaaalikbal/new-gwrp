...
public OnPlayerTeleport(playerid, Float:distance)
{
	...
	{
	    if(!IsPlayerInRangeOfPoint(playerid, 4.0, pData[playerid][pPosX], pData[playerid][pPosY], pData[playerid][pPosZ]))
	    {
		    pData[playerid][pACWarns]++;

		    if(pData[playerid][pACWarns] < MAX_ANTICHEAT_WARNINGS)
		    {
	    	    SendAnticheat(COLOR_RED, ""RED_E"[Anticheat]%s"WHITE_E"[%i] Diduga Mengunakan Teleport haking! "YELLOW_E"[distance: [%.1f].", ReturnName(playerid), playerid, distance);
	        	new dc[128];
				format(dc, sizeof(dc),  ""PINK_E"[AntiCheat]"PINK_E"%s[%s] kemungkinan teleport hack [Jarak: %.1f]", ReturnName(playerid), pData[playerid][pID], distance);
				SendDiscordMessage(3, dc);
			}
			else
			{
		    	SendClientMessageToAllEx(COLOR_RED, "[GreenwichBot]: %s"WHITE_E" telah dikick otomatis oleh %s, Alasan: Teleport hacks", ReturnName(playerid), SERVER_BOT);
		    	new dc[128];
				format(dc, sizeof(dc),  ""PINK_E"```\n<!> %s telah dikick otomatis oleh %s\nAlasan: Teleport Hack```", ReturnName(playerid), SERVER_BOT);
				SendDiscordMessage(3, dc);
		    	KickEx(playerid);
			}
		}
...
public OnPlayerAirbreak(playerid)
{
	if((AntiCheatKontol) && pData[playerid][pAdmin] < 2)
	{
		...
		else
		{
		    ...
		    SendClientMessageToAllEx(COLOR_RED, "[GreenwichBot]: %s"WHITE_E" telah dibanned otomatis oleh %s, Alasan: Airbreak", ReturnName(playerid), SERVER_BOT);
		    new dc[128];
...
NgecekCiter(playerid)
{
	if(gettime() > pData[playerid][pACTime])
	{
		...
		if((AntiCheatKontol) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER && GetVehicleSpeed(GetPlayerVehicleID(playerid)) > 350 && pData[playerid][pAdmin] < 2)
		{
			...
			{
                new dc[128];
                SendClientMessageToAllEx(COLOR_RED, "[BirminghamBOT]: %s"WHITE_E" telah dikick otomatis oleh %s, Alasan: Speed hacks", ReturnName(playerid), SERVER_BOT);
		...
		{
            new dc[128];
            SendClientMessageToAllEx(COLOR_RED, "[BirminghamBOT]: %s"WHITE_E" telah dikick otomatis oleh %s, Alasan: Jetpack hacks", ReturnName(playerid), SERVER_BOT);
		...
		if((AntiCheatKontol) && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
		{
			switch(GetPlayerAnimationIndex(playerid))
			{
			    case 958, 1538, 1539, 1543:
			    {
                    ...
                    {
                        new dc[128];
                        SendClientMessageToAllEx(COLOR_RED, "[GreenwichBot]: %s"WHITE_E" telah dikick otomatis oleh %s, Alasan: Flying hacks", ReturnName(playerid), SERVER_BOT);
			...
			GetPlayerArmour(playerid, armor);
  			...
  			if(!(gettime() - pData[playerid][pLastUpdate] > 5))
  			{
				if(floatround(armor) > floatround(pData[playerid][pArmour]) && gettime() > pData[playerid][pACTime] && gettime() > pData[playerid][pArmorTime] && pData[playerid][pAdmin] < 2)
				{
					...
					{
                        new dc[128];
                        SendClientMessageToAllEx(COLOR_RED, "[GreenwichBot]: %s"WHITE_E" telah dikick otomatis oleh %s, Alasan: Armor hacks", ReturnName(playerid), SERVER_BOT);
...
{
    SendStaffMessage(COLOR_RED, "[FAKESPAWN]%s telah Mengunakan Fakespawn", pData[playerid][pUCP]);
    KickEx(playerid);
    return 1;
}
