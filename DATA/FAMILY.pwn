...
{
	new dquery[2048];
	format(dquery, sizeof(dquery), "UPDATE familys SET name='%s', leader='%s', motd='%s', color='%d', extposx='%f', extposy='%f', extposz='%f', extposa='%f', intposx='%f', fSkin1='%d', fSkin2='%d', fSkin3='%d', fSkin4='%d', fSkin5='%d', intposy='%f', intposz='%f', intposa='%f', fint='%d'",
	fData[id][fName],
	fData[id][fLeader],
	fData[id][fMotd],
	fData[id][fColor],
...
Family_WeaponStorage(playerid, fid)
{
    ...
    ShowPlayerDialog(playerid, FAMILY_WEAPONS, DIALOG_STYLE_LIST, "Weapon Storage", string, "{27d23d}Select", "{e31d1d}Cancel");
    return 1;
}
 	...
 	if(rows)
  	{
		...
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_name_int(i, "fSkin1", fData[fid][fSkin][0]);
		    cache_get_value_name_int(i, "fSkin2", fData[fid][fSkin][1]);
		    cache_get_value_name_int(i, "fSkin3", fData[fid][fSkin][2]);
		    cache_get_value_name_int(i, "fSkin4", fData[fid][fSkin][3]);
		    cache_get_value_name_int(i, "fSkin5", fData[fid][fSkin][4]);
...
CMD:fedit(playerid, params[])
{
    ...
    else if(!strcmp(type, "skin", true))
    {
    	...
    	if(sscanf(string, "dd", idf, skinnya))
    		return Usage(playerid, "/fedit [id] [type] [id skin]");

    	if(idf < 1 || idf > 5) return Error(playerid, "Invalid type.");

    	fData[fid][fSkin][idf] = skinnya;

    	Family_Save(fid);
		Family_Refresh(fid);

		SendStaffMessage(COLOR_RED, "%s has adjusted the skin fam ID: %d.", pData[playerid][pAdminname], fid);
    }
 	...
 	if(rows)
  	{
		...
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, "List Famliys", string, "{27d23d}Enter", "");
	}
}
...
/*CMD:f(playerid, params[]) // RADIOFAM
{
    new text[128];
    
    if(pData[playerid][pFamily] == -1)
        return Error(playerid, "You must be in a family to use this command");
            
    if(sscanf(params, "s[128]", text) < 1)
        return Usage(playerid, "/f(family) [text]");

    if(strlen(text) > 128)
        return Error(playerid, "Text is too long.");

    if(!IsFamilyMember(playerid, pData[playerid][pFamily]))
        return Error(playerid, "You are not a member of this family.");
    
    SendFamilyMessage(playerid, text);
    return 1;
}

SendFamilyMessage(playerid, const message[])
{
    new fid = pData[playerid][pFamily];
    
    for(new i = 0; i < MAX_PLAYERS; i++)
    {
        if(IsPlayerConnected(i) && IsFamilyMember(i, fid))
        {
            SendClientMessage(i, COLOR_SBLUE, message);
        }
    }
}

IsFamilyMember(playerid, fid)
{
    if(playerid < 0 || playerid >= MAX_PLAYERS) //apakah playerid valid
        return 0;
        
    if(fid < 0) //apakah fid valid
        return 0;
        
    return pData[playerid][pFamily] == fid && fid != -1;
}*/
 	...
 	if(rows)
  	{
		...
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Family Info", string, "{27d23d}Enter", "");
	}
}
...
CMD:famint(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_FAMILY_INTERIOR, DIALOG_STYLE_LIST, "Pilih interior untuk teleport.", list, "{27d23d}Enter", "{e31d1d}Cancel");
	return 1;
}
