	...
	if(rows)
  	{
		...
		while(i < rows)
		{
			...
			AtmData[id][atmObjID] = CreateDynamicObject(1515, AtmData[id][atmX], AtmData[id][atmY], AtmData[id][atmZ], AtmData[id][atmRX], AtmData[id][atmRY], AtmData[id][atmRZ], AtmData[id][atmWorld], AtmData[id][atmInt], -1, 50.0, 50.0);
			format(str, sizeof(str), ""YELLOW_E"/ATM");
...
CMD:createatm(playerid, params[])
{
	...
	new str[128];
	AtmData[id][atmObjID] = CreateDynamicObject(1515, AtmData[id][atmX], AtmData[id][atmY], AtmData[id][atmZ], AtmData[id][atmRX], AtmData[id][atmRY], AtmData[id][atmRZ], AtmData[id][atmWorld], AtmData[id][atmInt], -1, 10.0, 10.0);
	format(str, sizeof(str), ""YELLOW_E"/ATM");
	AtmData[id][atmLabel] = CreateDynamic3DTextLabel(str, COLOR_LIGHTGREEN, AtmData[id][atmX], AtmData[id][atmY], AtmData[id][atmZ] + 0.3, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, AtmData[id][atmWorld], AtmData[id][atmInt], -1, 10.0);
	Iter_Add(ATMS, id);
...
{
	Atm_Save(id);
	SuccesMsg(playerid, "Atm berhasil di buat!");
	new str[150];
...
CMD:removeatm(playerid, params[])
{
	...
	if(Atm_BeingEdited(id)) return ErrorMsg(playerid, "Can't remove specified atm because its being edited.");
...
CMD:atmdentot(playerid, params[])
{
	new AtmInfo[560];
	format(AtmInfo,1000,"%s", GetName(playerid));
	PlayerTextDrawSetString(playerid, AtmTD[playerid][10], AtmInfo);
	format(AtmInfo,1000,"%d", pData[playerid][pBankRek]);
	PlayerTextDrawSetString(playerid, AtmTD[playerid][12], AtmInfo);
	...
	PlayerTextDrawSetString(playerid, AtmTD[playerid][32], AtmInfo);
   	...
   	format(AtmInfo,1000,"%s", GetName(playerid));
   	PlayerTextDrawSetString(playerid, AtmTD[playerid][38], AtmInfo);
   	for(new i = 0; i < 76; i++)
	{
