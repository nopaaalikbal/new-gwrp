#define MAX_PRIVATE_VEHICLE 2000
#define MAX_PLAYER_VEHICLE 3
new bool:VehicleHealthSecurity[MAX_VEHICLES] = false, Float:VehicleHealthSecurityData[MAX_VEHICLES] = 10000.0;
...
function OnVehBuyPV(playerid, pid, model, color1, color2, cost, Float:x, Float:y, Float:z, Float:a)
{
	...
	OnPlayerVehicleRespawn(i);
	new lstr[500];
	format(lstr, sizeof(lstr), "Anda telah membeli kendaraan seharga %s dengan model %s(%d)", FormatMoney(GetVehicleCost(model)), GetVehicleModelName(model), model);
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1545.033935, -2177.635498, 13.582808))
	    return ErrorMsg(playerid, "Kamu harus berada di Kantor Insurance!");

	new bool:found = false, msg2[512];
	format(msg2, sizeof(msg2), "Model\tPlate\tInsurance\tStatus\n");
	foreach(new i : PVehicles)
	{
		if(pvData[i][cClaim] == 1)
		{
			if(pvData[i][cOwner] == pData[playerid][pID])
			{
				...
				{
					format(msg2, sizeof(msg2), "%s%s\t%s\t%d\tCan't Claim yet\n", msg2, GetVehicleModelName(pvData[i][cModel]), pvData[i][cPlate], pvData[i][cInsu]);
					found = true;
				}
				else
				{
					format(msg2, sizeof(msg2), "%s%s\t%s\t%d\tCan Claim now\n", msg2, GetVehicleModelName(pvData[i][cModel]), pvData[i][cPlate], pvData[i][cInsu]);
					found = true;
				}
			}
		}
	...
	if(found)
		ShowPlayerDialog(playerid, DIALOG_INSURANCE, DIALOG_STYLE_TABLIST_HEADERS, "Insuranced Vehicle", msg2, "Select", "Close");
	else
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Vehicles", "There is no vehicle in the insurance.", "Close", "");
	return 1;
}
...
CMD:apv(playerid, params[])
{
		...
		if(found)
			...
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Greenwich - Kendaraan Player", "Player ini tidak memeliki kendaraan", "Close", "");
		return 1;
}
...
CMD:vrm(playerid, params[])
{
	...
	if((carid = Vehicle_Nearest(playerid)) != -1)
	{
		...
		{
    		ShowPlayerDialog(playerid, DIALOG_VRM, DIALOG_STYLE_LIST, "Greenwich - Menu Kendaraan", "Kunci\nLampu\nBagasi\nBuka/Tutup Hood\nBuka/Tutup Trunk", "Pilih", "Batal");
		}
...
CMD:myv(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_MYVEH, DIALOG_STYLE_TABLIST_HEADERS, "{7fffd4}Greenwich {ffffff}- Kepemilikan Kendaraan", CMDSString, "Pilih", "Batal");
	return 1;
}
...
CMD:engine(playerid, params[])
{
	...
	{	
	    ...
	    if(pData[playerid][pLagiDealer] == 1)
            return Info(playerid, "Anda sedang Beli kendaraan dilarang abuse");
	...
	if((carid = Vehicle_Nearest(playerid)) != -1)
	{
		if(Vehicle_IsOwner(playerid, carid))
		{
    		if(!pvData[carid][cLocked])
    		{
				...
				Error(playerid, "Mohon jauhkan kendaraan jika tidak bisa terbuka/tertutup");
				GetPlayerPos(playerid, X, Y, Z);
...
{
	new vehid, otherid;
	if(sscanf(params, "ud", otherid, vehid)) return InfoMsg(playerid, "/givepv [playerid/name] [vehid] | /v my(mypv) - for find vehid");
	if(vehid == INVALID_VEHICLE_ID) return ErrorMsg(playerid, "Invalid id");

	if(!IsPlayerConnected(otherid) || !NearPlayer(playerid, otherid, 4.0))
		return ErrorMsg(playerid, "Player tidak ada di kota atau tidak di dekatmu!");
		...
		if(vehid == pvData[i][cVeh])
		{
			if(pvData[i][cOwner] == pData[playerid][pID])
			{
				...
				if(vehid == nearid)
				{
					...
					if(count >= limit)
					{
						ErrorMsg(playerid, "Player ini terlalu banyak memiliki kendaraan!");
						return 1;
					}
					new abc[500];
					format(abc,sizeof(abc), "Anda memberikan kendaraan %s(%d) anda kepada %s.", GetVehicleName(vehid), GetVehicleModel(vehid), ReturnName(otherid));
					SuccesMsg(playerid, abc);
					new lstr[500];
					format(lstr, sizeof(lstr), "%s Telah memberikan kendaraan %s(%d) kepada anda.(/mypv)", ReturnName(playerid), GetVehicleName(vehid), GetVehicleModel(vehid));
				...
				else return ErrorMsg(playerid, "Anda harus berada di dekat kendaraan yang anda jual!");
			}
