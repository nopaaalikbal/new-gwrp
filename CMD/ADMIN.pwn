CMD:nikahan(playerid, params[])
{
    if(pData[playerid][pAdmin] < 1)
		return ErrorMsg(playerid, "Anda bukan administrator Greenwich");
    if(Nikahan == false)
    {
        Nikahan = true;
...
CMD:respawnfaction(playerid, params[])
{
	...
	new lstr[1024];
	format(lstr, sizeof(lstr), "ADMINISTRATOR | %s: {ffffff}Telah Merespawn semua kendaraan faction!", pData[playerid][pAdminname]);
    SendClientMessageToAll(COLOR_LGWRP, lstr);
    if(IsValidVehicle(pData[playerid][pKendaraanFraksi]))
	DestroyVehicle(pData[playerid][pKendaraanFraksi]);
	return 1;
}
CMD:respawndealer(playerid, params[])
{
    if(pData[playerid][pAdmin] < 3)
       return ErrorMsg(playerid, "Anda tidak bisa akses perintah ini!");

	new lstr[1024];
	format(lstr, sizeof(lstr), "ADMINISTRATOR | %s: {ffffff}Telah Merespawn semua kendaraan dealer bug!", pData[playerid][pAdminname]);
    SendClientMessageToAll(COLOR_LGWRP, lstr);
...
CMD:respawnjobs(playerid, params[])
{
	...
	new lstr[1024];
	format(lstr, sizeof(lstr), "ADMINISTRATOR | %s: {ffffff}Telah Merespawn semua kendaraan kerja!", pData[playerid][pAdminname]);
    SendClientMessageToAll(COLOR_LGWRP, lstr);
...
CMD:acmds(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Greenwich - Bantuan Admin", line3, "OK","");
	return true;
}
...
CMD:makequiz(playerid, params[])
{
	...
	if(!strcmp(tmp, "question", true, 8))
	{
		...
		SendClientMessageToAll(-1,"{ffff00}Anda bisa memberi jawaban dengan menggunakan /qa.");
		quiz = 1;
	}
...
CMD:qa(playerid, params[])
{
	...
	if (quiz == 0) return ErrorMsg(playerid, "Sayangnya tidak ada kuis dari admin server.");
	if (sscanf(params, "s[256]", tmp)) return SyntaxMsg(playerid, "/qa [jawaban]");
...
CMD:admins(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS,"Staff List | Kota Greenwich",location,"Tutup","");
	return 1;
}
CMD:bin(playerid, params[])
{
	for(new i = 0; i < MAX_TRASH; i++)
	{
 		if(IsPlayerInRangeOfPoint(playerid, 2.3, TrashData[i][TrashX], TrashData[i][TrashY], TrashData[i][TrashZ]))
		{
			...
			{
				SuccesMsg(playerid, "Anda mendapatkan botol!!");
...
CMD:aduty(playerid, params[])
{
	if(pData[playerid][pAdmin] < 1)
		if(pData[playerid][pHelper] == 0)
			return ErrorMsg(playerid, "Your Are Not Greenwich Staff");

	if(!strcmp(pData[playerid][pAdminname], "None"))
		return ErrorMsg(playerid, "You Must Sett The Admin Name!");
	...
	if(!pData[playerid][pAdminDuty])
    {
		if(pData[playerid][pAdmin] > 0)
		{
			...
			SendClientMessageToAllEx(COLOR_WHITE, "{ff0f00}[STAFF] {ffffff}:{007A1E} %s {BABABA}Now Onduty Staff With Name {FF0F00}%s", pData[playerid][pName], pData[playerid][pAdminname]);
			pData[playerid][AdminTag] = CreateDynamic3DTextLabel("Loading Nametag...", 0xFFFFFFFF, 0.0, 0.0, 0.1, NT_DISTANCE, .attachedplayer = playerid, .testlos = 1);
			SetTimer("UpdateNametagAdmin", 1000, false);
		}
		else
		{
			SetPlayerColor(playerid, COLOR_BLACK);
			pData[playerid][pAdminDuty] = 1;
			SetPlayerName(playerid, pData[playerid][pAdminname]);
			SendClientMessageToAllEx(COLOR_WHITE, "{ff0f00}[STAFF] {ffffff}:{007A1E} %s {ffffff}Now Onduty Staff With Name {FF0F00}%s", pData[playerid][pName], pData[playerid][pAdminname]);
			pData[playerid][AdminTag] = CreateDynamic3DTextLabel("Loading Nametag...", 0xFFFFFFFF, 0.0, 0.0, 0.1, NT_DISTANCE, .attachedplayer = playerid, .testlos = 1);
			SetTimer("UpdateNametagAdmin", 1000, false);
		}
    }
    else
    {
        ...
        SendStaffMessage(COLOR_RED, "{ff0f00}[STAFF] {ffffff}:{007A1E} %s {ffffff}Now Offduty Staff.", pData[playerid][pAdminname]);
...
CMD:asay(playerid, params[])
{
    ...
    SendClientMessageToAllEx(COLOR_RED,"{ff0f00}[STAFF] {ffffff}: [{007A1E}%s{ffffff}]{ff0000} "YELLOW_E"%s: "LG_E"%s", GetStaffRank(playerid), pData[playerid][pAdminname], ColouredText(text));
    return 1;
}
...
CMD:a(playerid, params[])
{
	...
	SendAdminMessage(COLOR_GREEN, "{3BBD44}(( [STAFFMSG] : %s %s: %s ))", GetStaffRank(playerid), PlayerData[playerid][pAdminname], params);
	return 1;
}
...
CMD:togooc(playerid, params[])
{
    ...
    if(TogOOC == 0)
    {
        SendClientMessageToAllEx(COLOR_RED, "{ff0f00}[SERVER] {ffffff}: {ffff00}Staff %s telah menonaktifkan global OOC chat.", pData[playerid][pAdminname]);
        TogOOC = 1;
    }
    else
    {
        SendClientMessageToAllEx(COLOR_RED, "{ff0f00}[SERVER] {ffffff}: {ffff00}Staff %s telah mengaktifkan global OOC chat (DON'T SPAM).", pData[playerid][pAdminname]);
        TogOOC = 0;
    }
    return 1;
}
...
CMD:o(playerid, params[])
{
    ...
    if(strlen(params) < 90)
    {
        ...
        {
            if(pData[playerid][pAdmin] == 1) SendClientMessageEx(i, COLOR_LGWRP, "[G] GREENWICH STAFF || %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
            else if(pData[playerid][pAdmin] == 2) SendClientMessageEx(i, COLOR_LGWRP, "[G] GREENWICH STAFF || %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
            else if(pData[playerid][pAdmin] == 3) SendClientMessageEx(i, COLOR_LGWRP, "[G] GREENWICH STAFF || %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
            else if(pData[playerid][pAdmin] == 4) SendClientMessageEx(i, COLOR_LGWRP, "[G] GREENWICH STAFF || %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
            else if(pData[playerid][pAdmin] == 5) SendClientMessageEx(i, COLOR_LGWRP, "[G] GREENWICH STAFF || %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
            else if(pData[playerid][pAdmin] == 6) SendClientMessageEx(i, COLOR_LGWRP, "[G] GREENWICH STAFF || %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
			else if(pData[playerid][pHelper] > 0 && pData[playerid][pAdmin] == 0)
			{
				SendClientMessageEx(i, COLOR_LGWRP, "[G] GREENWICH STAFF || %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
			}
            else
            {
                SendClientMessageEx(i, COLOR_PINK2, "[G] CIVILIAN || %s [%d]: {ffffff}%s", pData[playerid][pUCP], playerid, params);
            }
        }
    }
    else
...
CMD:g(playerid, params[])
{
	if(pData[playerid][pVip] < 1)
		return ErrorMsg(playerid, "Anda bukan VIP");
 	if(isnull(params))
        return SyntaxMsg(playerid, "/g [teks]");
	if(pData[playerid][pVip] == 1)
	{
		new lstr[1024];
		format(lstr, sizeof(lstr), "[VIP] %s: {ffffff}%s", pData[playerid][pName], params);
...
CMD:apm(playerid, params[])
{
	...
	SendClientMessageEx(userid, COLOR_LIGHTRED, "[APM] : Message From Admin : {FFFFFF}%s", text);
	SendAdminMessage(COLOR_LIGHTRED, "[APM] {FFFFFF}: {007A1E}%s {ffffff}Message To {007A1E}%s{FFFFFF}: %s", PlayerData[playerid][pUCP], GetName(userid), text);
...
CMD:arevive(playerid, params[])
{
    ...
    Info(playerid, "Admin %s Telah menyembuhkan player %s.", pData[playerid][pAdminname], ReturnName(otherid));
    Info(otherid, "Anda telah disembuhkan oleh admin %s", pData[playerid][pAdminname]);
    return 1;
}
...
CMD:spec(playerid, params[])
{
	...
	new otherid;
    if(sscanf(params, "u", otherid))
        return SyntaxMsg(playerid, "/spec [playerid/PartOfName] - Type '/spec off' to stop spectating.");
...
CMD:jail(playerid, params[])
{
	...
	if(reason[0] == '*')
	{
		SendClientMessageToAllEx(COLOR_RED, "FEDERAL | NEWS : {FFFFFF}%s success to arrested with reason -> %s is -> %d bulan.", pData[otherid][pName], reason, timeSec);
		new str[150];
...
CMD:unjail(playerid, params[])
{
	...
	SetPlayerInterior(otherid, 0);
	SetPlayerVirtualWorld(otherid, 0);
	SetPlayerPos(otherid, 1552.42,-1675.71,16.19);
	SetPlayerSpecialAction(otherid, SPECIAL_ACTION_NONE);

	SendClientMessageToAllEx(COLOR_PINK2, "BERITA | PENJARA: {FFFFFF}%s telah membebaskan pemain %s dari penjara", pData[playerid][pAdminname], pData[otherid][pName]);
	new str[150];
...
CMD:kick(playerid, params[])
{
    ...
    if(sscanf(params, "us[128]", otherid, reason))
        return SyntaxMsg(playerid, "/kick [playerid] [reason]");
	...
	SendClientMessageToAllEx(COLOR_LIGHTRED, "[STAFF] {FFFFFF}: %s {FFFFFF}telah ditendang oleh {ff0f00}%s\n{FF0F00}[REASON] {FFFFFF}: %s.", GetName(otherid), PlayerData[playerid][pAdminname]);
	SendClientMessageToAllEx(COLOR_LIGHTRED, "[REASON] {FFFFFF}: %s.", reason);
	KickEx(otherid);
    return 1;
}
...
CMD:ban(playerid, params[])
{
	...
	new zstr[325];
	format(zstr, sizeof(zstr),"{FFFFFF}UCP Anda diblokir dari Greenwich Roleplay\n\nBerdasarkan Informasi:\nNama UCP: {ffffff}%s\nIP Address: {FFFFFF}%s\nAdmin On Duty: {ffffff}%s\nTanggal Diblokir: {FFFFFF}%i/%02d/%02d %02d:%02d\nReason: {FFFFFF}%s\nDurasi Blokir: {ffffff}%s\n\n"YELLOW_E"INGAT: {FFFFFF}Jika anda ingin bermain lagi dan bertobat, masuk ke discord: "YELLOW_E"bit.ly/Greenwich {ffffff}kemudian pilih chanel #request � unbanned � Greenwich", pData[playerid][pUCP], PlayerIP, PlayerData[playerid][pAdminname], date[2], date[0], date[1], date[3], date[4], tmp);
	ShowPlayerDialog(otherid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "{007A1E}Greenwich {ffffff}/{007a1e}/ {ffffff}UCP Blocked", zstr, "{e31d1d}Close", "");
...
CMD:setaluminium(playerid, params[])
{
	if(pData[playerid][pAdmin] < 2)
        return ErrorMsg(playerid, "Anda tidak bisa akses perintah ini!");
	...
	new jumlah, otherid;
	if(sscanf(params, "ud", otherid, jumlah))
        return SyntaxMsg(playerid, "/setalumunium [playerid id/name] <jumlah>");
        
	if(!IsPlayerConnected(otherid))
        return ErrorMsg(playerid, "Player belum masuk!");

	pData[otherid][pAluminium] = jumlah;
	SuccesMsg(playerid, "Anda berhasil memberikan Gun!");
	return 1;
}
...
CMD:setweather(playerid, params[])
{
	...
	format(lstr, sizeof(lstr), "ADMINISTRATOR | %s: {ffffff}Telah Menganti Cuaca", pData[playerid][pAdminname]);
    SendClientMessageToAll(COLOR_PINK2, lstr);
	foreach(new ii : Player)
	{
    ...
    SuccesMsg(playerid, "Anda telah mengubah cuaca di kota Greenwich.");
    return 1;
}
...
CMD:reviveall(playerid, params[])
{
	...
	new lstr[1024];
	format(lstr, sizeof(lstr), "ADMINISTRATOR | %s: {ffffff}Telah Revive semua player yang ada di kota", pData[playerid][pAdminname]);
    SendClientMessageToAll(COLOR_PINK2, lstr);
	foreach(new pid : Player)
	{
...
CMD:setadminlevel(playerid, params[])
{
	...
	if(sscanf(params, "ud", otherid, alevel))
	{
	    SyntaxMsg(playerid, "/setadminlevel <ID/Name> <level 0 - 6>");
	    return true;
	}
	if(!IsPlayerConnected(otherid))
        return ErrorMsg(playerid, "Player belum masuk!");
	if(alevel > 10)
		return ErrorMsg(playerid, "Level can't be higher than 10!");
	if(alevel < 0)
...
CMD:setstat(playerid, params[])
{
	...
	if(IsPlayerConnected(playerid))
	{
		...
		else if(!strcmp(name, "phonecredit", true))
		{
			...
			mysql_format(g_SQL, query, sizeof(query), "UPDATE players SET phonecredit = %d WHERE reg_id = %d", pData[otherid][pPhoneCredit], pData[otherid][pID]);
	    	mysql_tquery(g_SQL, query);
		}
...
CMD:moderatorhelp(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "{007a1e}Moderator Commands | Greenwich Roleplay", string, "{e31d1d}Close","");
	return 1;
}
...
CMD:afuninvite(playerid, params[]) //famuninadmin
{		
	if(pData[playerid][pAdmin] < 4)
		return Error(playerid, "You Must Staff Rank 4 - 10!");
	
	new otherid;
    if(sscanf(params, "u", otherid))
        return Usage(playerid, "/fauninvite [playerid/PartOfName]");
	...
	SetPlayerSkin(otherid, pData[otherid][pSkin]);
